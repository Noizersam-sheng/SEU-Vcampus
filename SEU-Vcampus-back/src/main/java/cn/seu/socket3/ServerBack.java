...
public class ServerBack implements Runnable, Serializable {
    private Socket socket;

    public ServerBack(Socket socket) {
        this.socket = socket;
    }
    ...
    class Server_process implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                AliPayImpl al = new AliPayImpl();
                PollingImpl pl =new PollingImpl();
                ...
                Book book;
                AlipayInfoInDTO alipayInfoInDTO;
                AlipayResultInDTO alipayResultInDTO;
                if (state) {
                    switch (type) {
                        ...
                        case 526:
                            ...
                            break;
                        case 527:
                            object = cl.deleteStudentCourse(data.toString().split("\n")[0], data.toString().split("\n")[1]);
                            object = cl.updateCourseNum(data.toString().split("\n")[0], data.toString().split("\n")[1]);
                            break;
                        case 611:
                        ...
                        //进入springboot的船新版本
                        case 710:
                            if (data instanceof AlipayInfoInDTO) {
                                alipayInfoInDTO = (AlipayInfoInDTO) data;
                                object = al.createOrderForm(alipayInfoInDTO);
                            }
                            break;
                        case 711:
                            if (data instanceof AlipayResultInDTO) {
                                alipayResultInDTO = (AlipayResultInDTO) data;
                                object = pl.inquireTradeStatus(alipayResultInDTO);
                            }
                            break;
                        default:
                            break;
